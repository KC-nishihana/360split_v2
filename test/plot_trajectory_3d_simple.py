...
GRAVITY_WINDOW = 100  # 重力推定用の移動平均ウィンドウサイズ
...
# 移動平均で重力成分を推定
gravity_x = moving_average(acc_x, GRAVITY_WINDOW)
gravity_y = moving_average(acc_y, GRAVITY_WINDOW)
gravity_z = moving_average(acc_z, GRAVITY_WINDOW)
...
vel_x = remove_linear_trend(vel_x)
vel_y = remove_linear_trend(vel_y)
vel_z = remove_linear_trend(vel_z)
...
output_df = pd.DataFrame({
    ...
    "AccelY_g": acc_y,
    "AccelZ_g": acc_z,
    "GravityX_g": gravity_x,
    "GravityY_g": gravity_y,
    "GravityZ_g": gravity_z,
...
# 開始点と終了点をマーク
ax1.scatter(pos_x[0], pos_y[0], pos_z[0], c='green', s=100, marker='o', label='Start', edgecolors='black')
ax1.scatter(pos_x[-1], pos_y[-1], pos_z[-1], c='red', s=100, marker='x', label='End', linewidths=3)
...
scatter = ax2.scatter(pos_x, pos_y, c=time, cmap='viridis', s=10, alpha=0.6)
ax2.plot(pos_x[0], pos_y[0], 'go', markersize=10, label='Start')
ax2.plot(pos_x[-1], pos_y[-1], 'rx', markersize=10, label='End', markeredgewidth=2)
...
scatter = ax3.scatter(pos_x, pos_z, c=time, cmap='viridis', s=10, alpha=0.6)
ax3.plot(pos_x[0], pos_z[0], 'go', markersize=10, label='Start')
ax3.plot(pos_x[-1], pos_z[-1], 'rx', markersize=10, label='End', markeredgewidth=2)
...
scatter = ax4.scatter(pos_y, pos_z, c=time, cmap='viridis', s=10, alpha=0.6)
ax4.plot(pos_y[0], pos_z[0], 'go', markersize=10, label='Start')
ax4.plot(pos_y[-1], pos_z[-1], 'rx', markersize=10, label='End', markeredgewidth=2)
...
# 加速度
axes[0].plot(time, acc_dynamic_x_ms2, label='X', alpha=0.7, linewidth=0.8)
axes[0].plot(time, acc_dynamic_y_ms2, label='Y', alpha=0.7, linewidth=0.8)
axes[0].plot(time, acc_dynamic_z_ms2, label='Z', alpha=0.7, linewidth=0.8)
...
# 速度
axes[1].plot(time, vel_x, label='X', alpha=0.7, linewidth=0.8)
axes[1].plot(time, vel_y, label='Y', alpha=0.7, linewidth=0.8)
axes[1].plot(time, vel_z, label='Z', alpha=0.7, linewidth=0.8)
axes[1].set_ylabel('Velocity [m/s]')
axes[1].set_title('Velocity (Drift Removed)')
axes[1].legend()
axes[1].grid(True, alpha=0.3)

# 位置
axes[2].plot(time, pos_x, label='X', alpha=0.7, linewidth=0.8)
axes[2].plot(time, pos_y, label='Y', alpha=0.7, linewidth=0.8)
axes[2].plot(time, pos_z, label='Z', alpha=0.7, linewidth=0.8)
...
print(f"Final position: ({pos_x[-1]:.3f}, {pos_y[-1]:.3f}, {pos_z[-1]:.3f}) meters")
print(f"\nOutput files:")
print(f"  - {output_plot}")
print(f"  - trajectory_timeseries.png")
print(f"  - {output_csv}")
print(f"\n[NOTE] IMU-based position estimation has limited accuracy due to drift and noise.")
print(f"[NOTE] This is a simplified implementation without gyroscope fusion.")
print(f"[NOTE] For real applications, consider Kalman filtering or sensor fusion.")
